...
export interface SellerPayoutData {
  storeName: string;
  payoutId: string;
  amount: number;
  currency: string;
...
export function sellerPayoutSubject(data: SellerPayoutData) {
...
export function sellerPayoutHtml(data: SellerPayoutData) {
  const amount = `${data.amount.toFixed(2)} ${data.currency.toUpperCase()}`;
  const heading = data.status === "paid" ? "Payout Sent To Your Bank" : "Payout Failed";
  ...
  const body = `
    ...
    <tr>
      <td align="center" style="color:${colors.muted};font-size:14px;padding-bottom:8px;">
        Hi ${data.storeName}, here's an update from your GoWithPorto connected Stripe account.
      </td>
    </tr>
    ${metaBar([
