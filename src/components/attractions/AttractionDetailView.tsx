...
export default function AttractionDetailView({ attraction }: { attraction: Attraction }) {
  const [activeImage, setActiveImage] = useState(0);
  const { favorited, toggle: toggleFavorite } = useFavorite("attraction", attraction._id);
          ...
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-black/5 bg-gray-100 shadow-sm">
            {images[activeImage] && (
              <img
                ...
                className="gallery-fade-in h-full w-full object-cover"
              />
            )}
